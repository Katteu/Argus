...
public class CriticalNodezException(string message) : Exception(message) { }

public class TestWorker<T>(
    IConfiguration configuration,
    ILogger<TestWorker<T>> logger,
    ...
    private async Task ChainSyncReducerAsync(IReducer reducer, CancellationToken stoppingToken)
    {
        ...
        void Handler(object? sender, ChainSyncNextResponseEventArgs e)
        {
            ...
            logger.Log(
                ...
                TestWorker<T>.GetTypeNameWithoutGenerics(reducer.GetType()),
                response.Action,
                response.Block.Slot,
